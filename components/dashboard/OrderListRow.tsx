  ...
  return (
    <button
      ...
      className={`w-full rounded-xl border px-3.5 py-3 text-left transition ${
        selected
          ? 'border-brand-500/40 bg-brand-50/70 ring-1 ring-brand-500/30 dark:bg-brand-500/10'
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          <p className="truncate text-xs text-slate-400 dark:text-slate-500">{pedido.customer_phone || '—'}</p>
        </div>
        <span className="shrink-0 text-sm font-bold text-brand-800 dark:text-brand-300">{formatCurrency(pedido.total)}</span>
      </div>
